...
export = class Users extends Array {
    ...
    public static getList = ({country, limit, offset, order, ascending, modMode, hasPlayed}: QueryOptions) => {
        return new Promise((resolve, reject) => {
            const query =   `SELECT u.id userId, u.name, u.country, u.priv privilege, u.clan_id clanId, u.creation_time creationTime, s.playtime ` +
                            ...
                            `AS countryRank FROM stats s JOIN users u ON s.id = u.id WHERE u.priv >= 3 ${country ? `AND u.country = ?` : ""} ${hasPlayed ? `AND s.plays > 0` : ""} AND s.mode = ? ORDER BY ${order} ${(ascending) ? "ASC" : "DESC"}, playCount ${(ascending) ? "ASC" : "DESC"} LIMIT ?, ?; `;
    ...
    public static count = ({country, modMode, hasPlayed}: QueryOptions) => {
        return new Promise((resolve, reject) => {
            const query = `SELECT COUNT(*) as count FROM stats s JOIN users u ON s.id = u.id WHERE u.priv >= 3 ${country ? `AND u.country = ?` : ""} ${hasPlayed ? `AND s.plays > 0` : ""} AND s.mode = ?;`;
